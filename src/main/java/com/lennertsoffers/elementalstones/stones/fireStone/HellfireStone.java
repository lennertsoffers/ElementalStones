...
//        if (activePlayer.getFloatingFire() != null) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < 10; i++) {
                        Location locationOnCircle = Tools.locationOnCircle(location, 4, angle, player.getWorld());
                        locationOnCircle.setX(location.getX() + random.nextGaussian() / 10);
                        locationOnCircle.setY(location.getY() + random.nextGaussian() / 10);
                        locationOnCircle.setZ(location.getZ() + random.nextGaussian() / 10);
                        player.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, locationOnCircle, 0, 0, -0.5, 0);
                    }
                    if (tickCount > 200) {
                        this.cancel();
                    }
